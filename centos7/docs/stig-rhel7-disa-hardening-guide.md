...
This guide will not include any  command or configuration codes that you should execute or configure to achieve the security coplias. Instead it provides  you the appropriate links of the orriginal guide which contains the detailed descriptions, rationals, commands, and remediation scrpts that can be achieved a single goal. The intetion of this guide is to rovide an a automated solution to achieve the complies in an idemptent and productive approach. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzOTgwNjkwNzZdfQ==
-->