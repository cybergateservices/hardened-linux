...
The intention of this guide is to help you to harden your CentOS server using OpenSCAP tools. We are using the upstream DISA STIG security profile for Red Hat Enterprise Linux 7 which provides you a system which is equivalent to US Department of Defense systems security requirements  which will satisfy majority of your security compliances and beyond.
...
This guide will not include or repeat any  command or configuration codes that you should execute or configure to achieve the security compliances which are mentioned in the SCAP security guide. Instead it  directs you to the appropriate links of the original guide which contains the detailed descriptions, rationales, commands, and remediation scripts that can be used to achieve a single goal by using  a manual execution or configuration pertaining to a specific task .  The guide includes a very high level  description  of what tasks to be done to meet a particular  compliances of the security policy. 
## An Automated Approach 
The prime intention of this guide is to provide you  an an automated solution to achieve the compliances in an idempotent and productive nature.   We will be using an ```Ansible``` role to automate  majority  of  remediation  steps  which are required to meet the compliances required by the DISA STIG Profile. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNzc4MDIyMzE5XX0=
-->